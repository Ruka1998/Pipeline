Shader "Scene/Skybox"
{
    Properties
    {
        ...
        [Tex(g2, _Col1)][NoScaleOffset]_NoiseTex ("噪声贴图", 2D) = "white" { }
        [HideInInspector]_Col1 ("偏色", Color) = (1, 1, 1, 1)
        [Sub(g2)]_CutOff ("云层密度", Range(0, 1)) = 0.5
        [Sub(g2)]_CloudSize ("云层缩放", Range(0, 1)) = 1
        ...
        [Sub(g2)]_LitEdgePower ("LitEdgePower", Float) = 1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata_t v)
            {
                ...
                float2 uv = v.vertex.xz / v.vertex.y;
            ...
            half4 frag(v2f IN): SV_Target
            {
                ...
                if (y < 0.0)
                {
                    ...
                    fixed3 cloudCol = lerp(_Col1, _LightColor0, saturate(lightFinal));
                    cloudCol = lerp(0, cloudCol, -ray.y);
                    fixed3 sunCol = IN.sunColor * calcSunAttenuation(_WorldSpaceLightPos0.xyz, -ray);
                    col += lerp(sunCol, cloudCol, saturate(noise - _CutOff));
                }
                return half4(col, 1);
            }
            ENDCG
